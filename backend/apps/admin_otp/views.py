...
def _otp_cache_key(user_id: int) -> str:
    return f"admin_otp:{user_id}"


def send_otp(request):
    """관리자 로그인 후 OTP 발송.

    유효한 OTP가 캐시에 이미 있으면(만료 전) 재발송·재생성하지 않음.
    새로고침으로 인해 메일과 저장된 번호가 어긋나는 문제를 방지한다.
    """
    user_id = request.session.get("pre_otp_user_id")
    if not user_id:
        return redirect(_admin_login_url())

    cache_key = _otp_cache_key(user_id)
    expiry = getattr(settings, "OTP_EXPIRY_SECONDS", _OTP_CACHE_TTL)

    if cache.get(cache_key):
        return render(request, "admin_otp/otp_verify.html")

    otp_code = str(random.randint(100000, 999999))
    cache.set(cache_key, otp_code, timeout=expiry)
...
def verify_otp(request):
    ...
    if saved_code is None:
        return render(
            request,
            "admin_otp/otp_verify.html",
            {"error": "인증번호가 만료됐거나 없습니다. 관리자 로그인부터 다시 해주세요."},
        )
